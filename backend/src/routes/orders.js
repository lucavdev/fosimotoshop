...
router.post("/", verifyToken, async (req, res) => {
  ...
  try {
    ...
    // Bulk query to avoid N+1 query pattern
    ...
    for (const item of items) {
      ...
      orderItems.push({ productId: product.id, qty, price: product.price });
    }

    // Wrap in a transaction to guarantee atomicity (all succeed or none)
